...
// Bolt ⚡: To optimize performance, we avoid creating a new file watcher
// for every file when serving a directory. Instead, a single directory
// watcher is used in `serveDirectory`. The _suppressWatcher flag prevents
// redundant watchers when this function is called from serveDirectory.
FileServer.prototype.serveFile = function(fileName, mimeType = 'text/plain', maxAge = 0, _suppressWatcher) {
    ...
    if (!_suppressWatcher && !watchers[fileName]) {
        ...
        watcher.on('change', removeFile);
        watcher.on('unlink', () => {
            removeFile();
            delete watchers[fileName];
            delete fileServer.watchers[fileName];
        });
...
FileServer.prototype.serveDirectory = function(rootDirectory, mimeTypes, maxAge = 0) {
    ...
    // Bolt ⚡: To optimize performance, we create a single watcher per directory
    // instead of one for every file. This significantly reduces resource usage.
