...
FileServer.prototype.serveFile = function(fileName, mimeType = 'text/plain', maxAge = 0, watchFile = true) {
    const fileServer = this;

    // ⚡ Bolt: Only create a per-file watcher if explicitly enabled.
    if (watchFile && !watchers[fileName]) {
        const watcher = chokidar.watch(fileName, { persistent: true, ignoreInitial: true });
        watcher.on('change', () => {
            // ⚡ Bolt: When a file changes, invalidate both the original and gzipped versions from the cache.
...
FileServer.prototype.serveDirectory = function(rootDirectory, mimeTypes, maxAge = 0) {
    ...
    // ⚡ Bolt: Create a single watcher for the entire directory to save resources.
    if (!watchers[rootDirectory]) {
        ...
        const onFileChange = (changedPath) => {
            // ⚡ Bolt: When a file changes, invalidate both the original and gzipped versions from the cache.
            fileServer.cache.del(changedPath);
            fileServer.cache.del(`${changedPath}.gz`);
        };
