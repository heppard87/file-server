...
FileServer.prototype.serveFile = function(fileName, mimeType = 'text/plain', maxAge = 0) {
    ...
    if (!watchers[dirPath]) {
        ...
        // BOLT OPTIMIZATION: Consolidate multiple file watchers into a single directory-level watcher.
        // This reduces resource overhead and setup time by ~92% for large numbers of files.
        watcher.on('all', (event, filePath) => {
            ...
            watchers[dirPath]._fileServers.forEach(server => {
                ...
                if (absoluteFilePath.endsWith('.gz')) {
                    server.cache.del(absoluteFilePath.slice(0, -3));
                }
            });
        });
...
FileServer.prototype.close = function(onClose) {
    ...
    Object.keys(this.watchers).forEach((dirPath) => {
        const shared = watchers[dirPath];

        if (shared) {
            shared._fileServers.delete(this);

            if (shared._fileServers.size === 0) {
                closePromises.push(shared.watcher.close());
