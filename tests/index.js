...
test('serveFile 404s on not isFile', t => {
    ...
    const fileServer = new MockFileServer(createErrorCallback(t, {
        code: 404,
        message: `404: Not Found ${path.resolve(testFileName)}`,
    }));
...
test('serveFile watches files only once with chokidar', t => {
    ...
    mocks.chokidar.watch = (fileName, options) => {
        t.equal(fileName, path.resolve(path.dirname(testFileName)), 'got correct fileName');
...
test('close terminates all file watchers', t => {
    ...
    // BOLT: The files are in different directories ( /app and /app/files/bar ), so they have separate watchers.
